...
(function() {
    ...
    function parserAbstractFunction(key, fileName) {
        ...
        jsdom.env(fileContent, (err, window) => {
            ...
            else if(methods.length > 0 && classes.length > 0){
                for(let i=0;i<methods.length;i++) {
                    let methodLink = methods[i].getElementsByClassName('methodname').item(0).firstChild;
                    if(methodLink.constructor.name == 'HTMLAnchorElement')
                    {
                        let file = methodLink.getAttribute('href');
                        fileIterator(file);
                    }
                    if(debug) console.log('[L] Skipping : '+chalk.blue(fileName));
                }
                window.close()
                return;
            }
    ...
    main();
    //
    //fileIterator('exception.tostring.html');
    //fileIterator('mongocollection.--tostring.html');
})();
